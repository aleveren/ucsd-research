...
Stochastic variational inference accomplishes this by repeatedly sampling mini-batches of observations.
...
In other words, if the full dataset has $D$ observations, and a mini-batch contains $S$ observations, with $S \ll D$, we perform updates for the global parameters using a virtual dataset consisting of $D / S$ copies of the mini-batch.
This procedure is outlined in Algorithm~\ref{alg:svi}.
...
\item \textbf{Input:}
  \begin{itemize}
  \item $\vec x$: observed dataset of length $D$
  \item $\alpha$: vector of hyperparameters corresponding to the global variables
  \item $\{\rho_t\}_{t \geq 1}$: sequence of step-sizes satisfying $\sum_{t \geq 1} \rho_t = \infty$ and $\sum_{t \geq 1} \rho_t^2 < \infty$
  \end{itemize}
\item \textbf{Output:}
  \begin{itemize}
  \item $(\nu_{d,\ell})$: matrix of optimized parameters corresponding to the local latent variables. \\
  Here, $\nu_{d,\ell}$ corresponds to the $\ell\nth$ local variable of the $d\nth$ observation.
  \item $(\mu_1, \ldots, \mu_m)$: vector of optimized parameters corresponding to the global latent variables
  \end{itemize}
\item \textbf{Procedure:}
\end{itemize}
\begin{algorithmic}
\State Initialize global parameters $(\mu_1, \ldots, \mu_m)$ and the matrix of local parameters $(\nu_{d,\ell})$ to ``reasonable'' guesses based on the input data.
\While{ELBO has not converged}
  \State Randomly sample a mini-batch $b$ containing $S$ observations
  \For{each observation $x_i$ in $b$}
    \For{each local variable $\ell \in \{1, \ldots, L\}$}
      \State Update $\nu_{i,\ell} \gets \theta_\ell(E_{z \sim q}[\eta^{\text{local}}_{i,\ell}])$
    \EndFor
  \EndFor
  \State Set $\hat \mu$ to be a vector of $m$ zeros
  \For{each observation $x_i$ in $b$}
    \For{each global variable $k \in \{1, \ldots, m\}$}
      \State TODO***
      %\State $\hat \mu \gets \hat \mu + E_{z \sim q}[\eta^{\text{global}}_{TODO}]
    \EndFor
  \EndFor
  \State Update TODO***
  \State Compute ELBO according equation~\eqref{eq:elbo}.
\EndWhile
\State \Return $(\mu_1, \ldots, \mu_m)$ and matrix $(\nu_{d,\ell})$
\end{algorithmic}
\end{algorithm}
...
Next, we discuss a topic model which was defined in Griffiths et al \cite{griffiths2004hierarchical} and Blei et al \cite{blei2010ncrp} and which is capable of learning a hierarchy of topics.
...
The NCRP topic model is based on a probability distribution known as the Chinese Restaurant Process (CRP), which we will discuss first.
