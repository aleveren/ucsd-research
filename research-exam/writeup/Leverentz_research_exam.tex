...
% TODO: Should this be \theta_d, with another \phi_k ?
...
Each table has on it a card containing the address of another restaurant.
...
A single draw from the NCRP corresponds to an infinite sequence of restaurants.
Equivalently, we can think of the NCRP as a probability distribution over infinite sequences of positive integers.
...
There is some ambiguity in the terminology used in the literature, as the term ``NCRP'' can refer to the probability distribution defined above, but it has also been used to refer to a particular nested topic model defined in (**citation).
In (**so-and-so's) NCRP topic model, **TODO.
