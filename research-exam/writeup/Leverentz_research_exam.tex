...
\begin{tikzpicture}
\node[obs] (w) {$w_{d,n}$};
\node[latent, right=1.5cm of w] (phi) {$\phi_k$};
\node[latent, left=of w] (z) {$z_{d,n}$};
\node[latent, left=of z] (theta) {$\theta_d$};
\node[const, right=of phi] (alpha) {$\alpha$};
\node[const, left=of theta] (beta) {$\beta$};
...
\centeredPlate{word-plate}{(w)(z)}{$n \in \{1, \ldots, N_d\}$};
\centeredPlate{doc-plate}{(word-plate)(theta)}{$d \in \{1, \ldots, D\}$};
\centeredPlate{topic-plate}{(phi)}{$k \in \{1, \ldots, K\}$};
\end{tikzpicture}
%
...
c_d &\sim \mathrm{Categorical}(\{\pi_r\}) \\
t_{d,n} &\sim \mathrm{Categorical}(W[c_d[z_{d,n}]])
\shortintertext{where}
\pi_r &= \left( \prod_{\ell=1}^{L} V_{r[:\ell]} \right) \left( \prod_{\ell=1}^{L} \prod_{j=1}^{r[\ell]-1} (1 - V_{r[:\ell-1],j}) \right)
\end{align*}
