...
\1 For each document $d \in \{1, \ldots, D\}$:
  ...
  \2 For each word-slot $n \in \{1, \ldots, N_d\}$:
    ...
    \[ z_{d,n} \sim \text{Categorical}(\phi_d). \]
    \3 Let $r_d = \ell_{d,n}[1:z_{d,n}]$ represent the node at depth $z_{d,n}$ along the path from the root to the leaf $\ell_{d,n}$.  (Here, we allow $z_{d,n}$ to be $0$, corresponding to the root node.)
