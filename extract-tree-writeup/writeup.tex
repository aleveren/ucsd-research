...
\begin{itemize}
\item Let us say that a rooted, directed tree is \emph{admissible} if it has no nodes with exactly one child.
If $T$ is any tree, then $T$ satisfies the set of triplet constraints $C_T$ defined above.
...
{\bf TODO: Proof}
%
\item TODO: Can we also recover all of the Dirichlet parameters $\alpha_{\nu,\nu'}$ with some bounded level of accuracy?
(Need an algorithm to do so?)
%
...
%
\item Next, we'll show that both of the bounds $\alphaMax$ and $\deltaMin$ are necessary to prevent $\phi$ from becoming arbitrarily small.
